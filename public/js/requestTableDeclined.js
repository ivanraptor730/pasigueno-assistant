        ...
        if (user) {
            ...
            ref.once('value', function (snapshot) {
                ...
                return firebase.database().ref('users/' + parentKey).once('value').then(function (snapshot) {
                    ...
                    var rootRef = firebase.database().ref("request/forms").orderByChild("Barangay_Status").equalTo(barangay + "_Declined");
...
var category1 = document.getElementById("formsDenied");
function onChangeFormsDenied(){
...
function requestUtilsTableDeclined() {
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            var userId = firebase.auth().currentUser.uid;
            var users = firebase.database().ref('users');
            ...
            ref.once('value', function (snapshot) {
                ...
                return firebase.database().ref('users/' + parentKey).once('value').then(function (snapshot) {
                    barangay = (snapshot.val() && snapshot.val().Barangay) || 'Unknown';
                    var rootRef = firebase.database().ref("request/services").orderByChild("Barangay_Status").equalTo(barangay + "_Declined");
                    rootRef.on("value", snap => {
                        if (snap.exists()) {
                            $("#table_body1").html("");
                            snap.forEach(snap => {
...
var category2 = document.getElementById("utilDenied");
function onChangeFormsUtilsDenied(){
  var categorys = category2.value;
  if(categorys=="--"){
    
window.onload = requestUtilsTableDeclined();
  }
 else{
  firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
